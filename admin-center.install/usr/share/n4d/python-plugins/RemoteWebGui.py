...
class RemoteWebGui:
    ...
    def create_connection_vnc(self, id, options):
        try:
            ...
            self.machineid=id
            needle = False
            with open(chroot + "/etc/hosts", 'r') as fd:
                lines = fd.readlines()
                for x in lines:
                    if "127.0.0.1 " + self.machineid == x.strip():
                        needle = True
                        break

            if not needle:
                with open(chroot + "/etc/hosts", 'a') as fd:
                    fd.write("127.0.0.1 " + self.machineid + "\n")

            self.nspawn = subprocess.Popen("/usr/bin/systemd-nspawn -E XDG_CONFIG_DIRS=/home/root/.config/kdedefaults:/etc/xdg/lliurex/classroom:/etc/xdg/lliurex/layouts/current:/etc/xdg/lliurex/desktop:/etc/xdg/xdg-plasma:/etc/xdg -E SHELL=/bin/bash -E LANG=es_ES.utf8 --private-users=0 --private-users-chown --hostname={} -M {} -D{} tigervncserver -SecurityTypes None -depth 24 -geometry 1024x768 -fg -xstartup {}".format(id,id,chroot,startup),shell=True)
            retry = 3
            while retry>0:
                try:
